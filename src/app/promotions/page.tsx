...
export default function PromotionsPage() {
  return (
    <div className="min-h-screen bg-black">
      ...
      {/* SharpMoney March Madness / MLB Promo */}
      <section className="pb-12 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Active Promo Card */}
          <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-cyan/10 via-card-bg to-card-bg border-2 border-cyan shadow-[0_0_40px_rgba(0,229,255,0.15)]">
            {/* Live badge */}
            <div className="absolute top-4 right-4 flex items-center gap-2 bg-red-500/20 border border-red-500/40 text-red-400 text-xs font-bold px-3 py-1 rounded-full z-10">
            ...
            <div className="p-8 md:p-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                {[
                  { name: 'Core', old: '$29.99', price: '$14.99', href: 'https://whop.com/sharpmoney/core-ae/' },
                  { name: 'Pro', old: '$79.99', price: '$39.99', popular: true, href: 'https://whop.com/sharpmoney/pro-7e/' },
                  { name: 'Alpha', old: '$199.99', price: '$99.99', href: 'https://whop.com/sharpmoney/alpha-4e/' },
                ].map((plan) => (
                  <a
                    key={plan.name}
                    href={plan.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`block rounded-xl p-6 text-center transition-all hover:scale-[1.03] ${
                      plan.popular 
                        ? 'bg-cyan/10 border-2 border-cyan/40 shadow-[0_0_20px_rgba(0,229,255,0.1)]' 
                        : 'bg-white/5 border border-white/10 hover:border-white/20'
                    }`}
                  >
                    {plan.popular && (
                      <div className="text-xs font-bold text-cyan uppercase tracking-wider mb-2">Most Popular</div>
                    )}
                    ...
                    <div className="text-xs text-white/40">/first month</div>
                  </a>
                ))}
              </div>
              ...
              <div className="bg-white/5 rounded-xl p-6 mb-8">
                <h4 className="text-sm font-semibold text-white/50 uppercase tracking-wide mb-4">What&apos;s New</h4>
                <div className="grid sm:grid-cols-2 gap-3">
                  {[
                    'Full web + mobile platform',
                    'Fastest +EV engine on the market',
                    'Historical odds charts with Pinnacle limits',
                    'One-click deep links to bet slips',
                    'Advanced per-book filters',
                    'SharpMoney Signal — 5.4% ROI (Alpha)',
                  ].map((item, i) => (
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <span className="bg-cyan/10 border-2 border-cyan/30 text-cyan font-bold text-2xl tracking-widest px-6 py-3 rounded-lg">Madness50</span>
                </div>
                <div className="flex flex-col items-center sm:items-end gap-2">
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 bg-cyan text-black font-bold px-8 py-4 rounded-lg hover:bg-cyan-dim transition-all text-lg"
                  >
                    Claim 50% Off
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
